...
${ender}"
  ...
  echo "" >> "$bashrc"
  echo "$block" >> "$bashrc"
  init_log "UTF-8 编码配置已写�?$bashrc (OS=$OS_TYPE)"

  if [ "$OS_TYPE" = "windows" ]; then
    setx LANG "zh_CN.UTF-8" >> "$LOG_FILE" 2>&1
    setx LC_ALL "zh_CN.UTF-8" >> "$LOG_FILE" 2>&1
    init_log "Windows 系统环境变量 LANG/LC_ALL 已设�?
  fi
}
