...
resolve_python() {
  ...
  skill_log "WARN" "未找�?Python，执�?init.sh 自动下载..."
  bash "$PROJECT_DIR/\.codex/init.sh" >&2
  if [ -x "$EMBED_PYTHON" ]; then
    skill_log "INFO" "下载完成，使�? $EMBED_PYTHON"
    echo "$EMBED_PYTHON"
    return
  fi
