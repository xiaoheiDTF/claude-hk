#!/bin/bash
# skill-inject: 从用户 prompt 提取 skill 名，匹配注册表后注入上下文
# 被 03-user-prompt-submit/base.sh 调用
# 参数: $1 = 用户 prompt, $2 = session_id（来自 hook 输入 JSON）
#
# 匹配规则:
#   - 首字符必须是 /
#   - skill 名到第一个空格结束
