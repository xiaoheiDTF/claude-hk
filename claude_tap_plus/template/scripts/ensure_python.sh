...
# 被 init.sh 和 01-session-start 调用
#
# 策略:
#   1. 项目嵌入版（最优先，不依赖系统）
#   2. 系统 Python（用户已安装）
#   3. 自动下载（兜底，多源 + 重试）
#
# 状态文件: .claude/.python-state（纯文本，无需 Python 解析）
...
ensure_python() {
  ...
  case "$status" in
    ready)
      # 状态说是 ready 但实际找不到 → 重置
      _state_set status "not_tried"
      _state_set attempts "0"
      ;;
    failed_permanent)
      echo "[ensure_python] 已达最大重试次数，请手动安装 Python"
      echo ""
      return 1
      ;;
    downloading)
  ...
  if [ "$attempts" -ge "$MAX_ATTEMPTS" ]; then
    ...
    echo "[ensure_python] 已尝试 $attempts 次，停止重试"
    echo "[ensure_python] 请手动安装 Python 并重启 Claude Code"
    echo ""
    return 1
  fi
  ...
  echo "[ensure_python] Python 未找到，尝试自动下载 (第 $((attempts + 1))/$MAX_ATTEMPTS 次)..."
