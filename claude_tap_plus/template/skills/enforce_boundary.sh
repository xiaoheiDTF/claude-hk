...
# 读取 .active 中当前 session 的 skill，解析 SKILL.md frontmatter allowed-tools，
# 比对当前 tool_name，不在白名单则 deny
...
enforce_boundary() {
  ...
  # 提取 allowed-tools 列表（frontmatter 中 "  - ToolName" 格式的行）
  local allowed=""
  ...
  hook_output 2 "{\"hookSpecificOutput\":{\"hookEventName\":\"PreToolUse\",\"permissionDecision\":\"deny\",\"permissionDecisionReason\":\"${current_tool} 不在 ${skill_name} 的 allowed-tools 白名单中\"}}"
}
