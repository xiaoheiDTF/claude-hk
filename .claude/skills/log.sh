#!/bin/bash
# skills 统一日志模块
# 写入两处: hooks/logs (统一) + skills/log (按模块)
# 用法: 先设 SKILL_TAG，再 source 此文件
...
# 统一日志: .claude/hooks/logs/<日期>.log
UNIFIED_LOG_DIR="$PROJECT_DIR/.claude/hooks/logs"
mkdir -p "$UNIFIED_LOG_DIR"
UNIFIED_LOG_FILE="$UNIFIED_LOG_DIR/$(date +%Y-%m-%d).log"

# 模块日志: .claude/skills/log/<skill-tag>/<日期>.log
MODULE_LOG_DIR="$PROJECT_DIR/.claude/skills/log/$SKILL_TAG"
