...
**为什么要脚本而不是直接 npm run test:cdd：**
- 从 IDE 或非项目根目录启动时路径会错
- 脚本自动处理 Storybook 启停，用户不需要额外操作
- 脚本保证无论从哪里启动，日志都写到正确位置

### 框架配置（参考）

`package.json` scripts 和 Storybook 配置仍然保留，脚本和框架配置互为保底。
...
{
  "scripts": {
    "test:cdd": "test-storybook --output-dir .test-output/cdd/raw",
    "test:cdd:report": "node scripts/cdd-log-organizer.mjs"
  }
}
```

**Storybook 配置（`.storybook/test-runner.ts`）：**

```typescript
const config = getTestRunnerConfig();
config.screenshots = { enabled: true, directory: '.test-output/cdd/screenshots' };
```
